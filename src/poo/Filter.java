 ...
 * @version JDK 11.0.13
 * @see Cloneable interface foi neste caso usada para clonar arrayList
 */
public class Filter implements Cloneable {
	 ...
	 * 
	 * @param expression String array que contem a operacao logica ou relacional a
	 * ser estudada
	 */
	 ...
	 * getters e setters
	 */
	ArrayList<HashMap<String, String>> getResultingTable() {
		return this.resultingTable;
	}

	private void setResultingTable(ArrayList<HashMap<String, String>> resultingTable) {
		this.resultingTable = resultingTable;
	}
	...
	/*
	 * metodo para realizar a união ou interseção das arrayList na arrayList
	 * resultingTable.
	 */
	void logical() {
		...
		relational(getExpression()[1], 1);
		ArrayList<HashMap<String, String>> helpingTable = (ArrayList<HashMap<String, String>>) getResultingTable()
				.clone();
		resultingTable.clear();

		relational(getExpression()[5], 5);
		ArrayList<HashMap<String, String>> helpingTable2 = (ArrayList<HashMap<String, String>>) getResultingTable()
				.clone();
		resultingTable.clear();
	...
	void minor(int index) {
		for (int i = 0; i < getQueryTable().size(); i++)
			if (Double.parseDouble(getQueryTable().get(i).get(getExpression()[index - 1])) < Double
					.parseDouble(getExpression()[index + 1]))
				resultingTable.add(getQueryTable().get(i));
	}
	...
	/*
	 * metodo para verificar se cada linha da tabela é menor ou igual ao valor
	 * indicado
	 * 
	 * @param index da posicao do operador relacional
	 */
	...
	/*
	 * metodo para verificar se cada linha da tabela é maior ou igual ao valor
	 * indicado
	 * 
	 * @param index da posicao do operador relacional
	 */
